...
const TransformationForm = ({ action, data = null, userId, type, creditBalance, config }: TransformationFormProps) => {
  ...
  const onInputChangeHandler = (fieldName: string, value: string, type: string, onChangeField: (value: string) => void) => {
    debounce(() => { }, 1000); //waits a specific amount of time before it sends the input - does not get triggered upon every key-stroke
            ...
            <Select
              ...
              <SelectContent>
                {Object.keys(aspectRatioOptions).map((key) => (
                  <SelectItem key={key} value={key} className="select-item" >{key}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          )} />}
        {(type === "remove" || type === "recolor") && (
          <div className="prompt-field">
            <CustomField
              control={form.control}
              name="prompt"
              formLabel={"remove" ? "Object to remove" : "Object to recolor"}
              className="w-full"
              render={(({ field }) => (
                <Input
                  className="input-field"
                  value={field.value}
                  onChange={(e) => onInputChangeHandler('prompt', e.target.value, type, field.onChangeField)}
                />
              ))}
            />
          </div>
